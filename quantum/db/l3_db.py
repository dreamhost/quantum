...
class L3_NAT_db_mixin(l3.RouterPluginBase):
    ...
    def _get_router_for_floatingip(self, context, internal_port,
        ...
        router_intf_qry = context.session.query(models_v2.Port, RouterPort)
        router_intf_ports = router_intf_qry.filter(
            models_v2.Port.network_id==internal_port['network_id'],
            RouterPort.port_type==DEVICE_OWNER_ROUTER_INTF
        )

        for intf_p, routerport in router_intf_ports:
            subnet_ids = (i['subnet_id'] for i in intf_p['fixed_ips'])

            if internal_subnet_id in subnet_ids and routerport.router.gw_port:
